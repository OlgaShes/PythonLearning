...
def play(word):
    ...
    guessed_words = []
    tries = 6
    print('Угадай слово, которе я загадал!')
    print(display_hangman(tries))
    print(word_completion)
    while tries > 0  and guessed is False:
        my_try = input('Введите букву или слово целиком: ').upper()
        if my_try.isalpha() is False:
            print('Ошибка, можно вводить только буквы. Попробуйте еще раз :)')
            continue
        if len(my_try) == 1:
            if my_try in guessed_letters:
                print('Вы уже вводили эту букву, пропробуйте другую')
                continue
            else:
                guessed_letters.append(my_try)
                if my_try in word:
                    ...
                    print('Неверно, попробуй другую букву.')
                    tries -= 1
                    print(display_hangman(tries))
                    print(word_completion)
        else:
            if my_try in guessed_words:
                print('Вы уже вводили это слово, пропробуйте еще раз')
                continue
            else:
                guessed_words.append(my_try)
                if my_try == word:
                    guessed = True
                    break
                else:
                    ...
                    print(word_completion)
    if guessed is True:
        print(f'Поздравляем! Вы угадали, это было слово "{word}"')
    else:
        print(f'Увы, попытки исчерпаны, было загадано слово "{word}"')

print('Добро пожаловать в Балду')
...
if answer.lower() == "да":    
    while True:
        word = get_word()
        play(word)
        print('Спасибо, что играли в Балду. Хотите сыграть еще раз? (да / нет)')
        if input().lower() == "да":
            continue
        else:
